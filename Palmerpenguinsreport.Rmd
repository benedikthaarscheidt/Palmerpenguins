...
output:
  ...
  html_document:
    ...
    toc_depth: 5
    toc_float: yes
bibliography: kim14,haarscheidt,akinfalabi-penguinreport-reference.bib
---
...
In the intricate web of the natural world, the phenomenon of sexual dimorphism stands out as a captivating manifestation of evolutionary diversity. Just like in the species _homo sapiens_, the animal kingdom showcases a myriad of fascinating adaptations that highlight the distinct physical and behavioral differences between males and females of the same species. Sexual dimorphism, rooted in the forces of natural selection and reproductive strategies, offers a window into the adaptive mechanisms that have shaped countless species over millennia.  
...
Based on the distinctive traits observed in Chinstrap penguins and the evolutionary pressures associated with their challenging environments, we hypothesize that there exists a significant degree of sexual dimorphism in this species. We anticipate that our investigation will reveal measurable differences in body mass, flipper length, and bill size, providing insights into the ecological and reproductive roles played by male and female Chinstrap penguins.
...
In the following section, a series of statistical analyses were undertaken to investigate sexual dimorphism in chinstrap penguins. These analyses encompassed the computation of various statistical parameters for bill length and depth, body mass, and flipper length. Parameters such as arithmetic means, medians, and comprehensive summaries were derived, and these will be subsequently utilized in additional statistical analyses or tests. These parameters, will be presented in the results section, contributing to our understanding of sexual dimorphism in chinstrap penguins.
...
chinshapiro=rbind(chinshapiromale,chinshapirofemale)
```
To meet the assumption of normal distribution, we identified and removed an outlier from bill length of female chinstrap penguins dataset. This step was taken to enhance the normality of the data and ensure the validity of subsequent statistical analyses.

### T-test

Having rigorously assessed the normality of our dataset variables through Shapiro-Wilk tests, our analytical focus now shifts to conducting two-sample t-tests. This statistical method enables us to explore potential differences in specific physical characteristics between distinct groups, such as male and female penguins within each species.
...
Hypotheses:
To frame our analysis, we establish clear hypotheses. The null hypothesis posits no significant difference between male and female penguins, while the alternative hypothesis anticipates a significant distinction.

Significance Level:
Our statistical inferences will be based on a predetermined alpha level (0.05), and p-values resulting from the t-tests will guide our determination of statistical significance.

Application:
We apply these t-tests specifically to explore potential differences in physical characteristics between male and female Chinstrap penguins. Considering the normality status of our variables is crucial, as the validity of t-tests relies on the assumption of normal distribution. Insights gained from the Shapiro-Wilk tests inform the selection of appropriate statistical analyses.

By employing these robust statistical techniques, we aim to uncover nuanced variations in physical attributes, contributing to a comprehensive understanding of sexual dimorphism within penguin species.
...
In order to explore the potential relationship between sex and various physical characteristics of Chinstrap penguins, namely Body Mass, Flipper Length, Bill Length, and Bill Depth, we employed the chi-square test.
...
Hypothesis:
Our investigation aims to determine if there is a significant association between the sex of Chinstrap penguins and their respective Body Mass, Flipper Length, Bill Length, and Bill Depth.

Application:
We apply the chi-square test to assess the independence of the observed frequencies within the specified categories. By comparing observed and expected frequencies, we aim to unveil any significant patterns or deviations that may indicate a relationship between sex and the selected physical characteristics.

Interpretation:
The resulting chi-square statistic will be evaluated against critical values to determine statistical significance. Any observed differences between the expected and observed frequencies will guide our understanding of the potential dependence between sex and the physical attributes of Chinstrap penguins.

This statistical approach enhances our analysis by providing insights into the interplay between sex and various characteristics, contributing to a comprehensive exploration of sexual dimorphism within the Chinstrap penguin population.
...
**Table: Arithmetic Means of Physical Characteristics (Male vs Female)**

```{r echo=FALSE,results='asis'}
knitr::kable(table_data,col.names = c("Body mass (g)","Flipper length (mm)","Culmen length (mm)","Culmen depth (mm)"),digits = 3,align = c("c","c","c","c"))
```
...
**Table: SD (standard deviation from Mean) of Physical Characteristics (Male vs Female)**

```{r echo=FALSE,results='asis'}
knitr::kable(sdtable_data,col.names = c("Body mass (g)","Flipper length (mm)","Culmen length (mm)","Culmen depth (mm)"),digits = 3,align = c("c","c","c","c"))
...
For each pertinent variable, we executed a Shapiro-Wilk test, wherein the null hypothesis (H0) posits that the data follows a normal distribution, while the alternative hypothesis (H1) suggests a significant departure from normality.
...
knitr::kable(chinshapiro,col.names = c("Culmen length (mm)","Culmen depth (mm)","Flipper length (mm)","Body mass (g)"),align = c("c","c","c","c"),digits = 3)

```

### T Test

Following the comprehensive assessment of normality using Shapiro-Wilk tests, our attention shifts to the outcomes of two-sample t-tests, aimed at unraveling potential differences in specific physical characteristics between different penguin groups, specifically between males and females within each species.

Consideration of Normality:
Before delving into the t-test results, it is crucial to acknowledge the normality status of our variables. The validity of t-tests relies on the assumption of normal distribution, and the insights gained from the Shapiro-Wilk tests informed our selection of appropriate statistical analyses.

Summary of Findings:
The results of the two-sample t-tests are presented below, highlighting the mean values for female and male penguins within each species, along with the associated p-values. The p-value is a critical metric that indicates the probability of obtaining observed differences if there were no actual differences in the population. A smaller p-value suggests stronger evidence against the null hypothesis of no difference.
...
The p-values obtained for all categories are significantly small (all less than 0.05), indicating strong evidence to reject the null hypothesis. Therefore, we conclude that there are statistically significant differences in Culmen Length, Culmen Depth, Flipper Length, and Body Mass between female and male penguins within each species.
 ecology.
 
### Quantile splitting

In the analysis of the penguin dataset, we employed segmentation based on quantiles for key morphological characteristics, specifically body weight, bill length, bill depth, and flipper length. This segmentation strategy aimed to categorize individuals into distinct groups, allowing for a more detailed examination of the distribution of features within the population.

The tables below illustrate the distribution of penguins across different quantiles for each morphological trait, categorized by gender. Subsequently, this segmentation facilitated the application of further statistical tests, such as the Chi-Square test and the Cohen’s d test, to explore potential associations and differences between groups.


Table for the quantiles of the bodymass:
```{r echo=FALSE}
knitr::kable(tab)
```
Observation: The table reveals clear sexual dimorphism in body weight, with male penguins tending to be heavier on average compared to females.

<br>

Table for the quantiles of the bill length:
```{r echo=FALSE}
knitr::kable(tab.blen)
```
Observation: Sexual dimorphism is evident in bill length, with males generally exhibiting longer bills compared to females, as indicated by the higher counts in the "long" category for males.

<br>


Table for the quantiles of the bill depth:
```{r echo=FALSE}
knitr::kable(tab.bdep)
```
Observation: The table highlights sexual dimorphism in bill depth, with males showing a preference for deeper bills compared to females, as evidenced by the higher counts in the "deep" category for males.
<br>



Table for the quantiles of the flipper length:
```{r echo=FALSE}
knitr::kable(tab.flen)
```
Observation: Sexual dimorphism is observable in flipper length, with males generally having longer flippers than females, as indicated by the higher counts in the "long" category for males.

These segmented tables provide a foundation for the subsequent application of statistical tests, allowing for a more in-depth exploration of the relationships and differences in penguin morphological traits across different quantiles. The Chi-Square test will help assess the independence of categorical variables, while the Cohen’s d test will quantify the effect size of observed differences.


 
### Effect size

In our pursuit of a comprehensive understanding of observed differences, we turn our attention to the concept of effect size, exemplified by metrics such as Cohen’s d in our two-sample t-tests. Effect size is a pivotal component in statistical analyses, offering a standardized measure that transcends the boundaries of statistical significance. In the realm of Cohen’s d, this metric is calculated by dividing the difference between group means by the pooled standard deviation, expressing the extent of group differences in terms of standard deviations. Importantly, this standardized unit of measurement is independent of variable scales, facilitating meaningful comparisons across studies.

Interpreting Cohen’s d:
Guidelines for interpreting Cohen’s d provide a qualitative assessment of effect size. A small effect size hovers around 0.2, a medium size is approximately 0.5, and a large size is considered 0.8 or higher. The sign of Cohen’s d conveys the direction of the effect: positive values imply a higher mean in the first group, while negative values suggest the opposite.
...
# Print the table
knitr::kable(cohen_d_table,align = c("l","c","c"),col.names = c("","Cohen's d","Effect Size"))
```

Interpretation:
The resulting Cohen’s d values, computed using the cohen.d function from the effsize library, offer profound insights into the standardized differences between male and female Chinstrap penguins for each physical characteristic. In our context, the consistently large effect sizes across Body Mass, Culmen Length, Culmen Depth, and Flipper Length suggest substantial and practically significant distinctions between male and female penguins within each species.

Significance Beyond Statistical Tests:
This move beyond statistical significance enables us to assess the real-world impact of these variations in physical characteristics. By considering effect sizes, we gain a deeper understanding of the practical significance of observed disparities, complementing our inferential statistics and enriching the interpretation of the nuanced differences among the studied attributes.


### Chi-Square test

In our meticulous analysis of chinstrap penguin data, our focus was to discern the intricate association between sex and body size, specifically categorized into three groups: small, medium, and large. The parameters of interest encompassed bill length, bill depth, flipper length, and body mass. To ensure a balanced representation, we leveraged quantiles for the categorization process.



```{r echo=FALSE,results='hide'}
bodypart = c("Body mass","Bill length","Bill depth","Flipper length")
pval.bodypart = c("1.74e-04","2.07e-09","2.07e-09","1.26e-09")
stsig= c("yes","yes","yes","yes")
dbodypart = data.frame(bodypart,pval.bodypart,stsig)

```
```{r echo=FALSE,results='asis'}
knitr::kable(dbodypart,digits = 10
             ,col.names = c("","p-value","Statistical Significance")
             ,align = c("l","c","c"))
```

The chi-square independence test results reveal compelling evidence of a statistically significant association between sex and the categorized size groups for all measured attributes. Notably, the p-values across all parameters are exceptionally low (all less than 0.001), indicating a high level of statistical significance.

Body Mass:    
The observed association between sex and body mass categories is highly significant, suggesting that the distribution of penguins across small, medium, and large size groups significantly differs between males and females.

Bill Length:    
The chi-square test underscores a highly significant relationship between sex and bill length categories. This finding implies a distinct distribution pattern of penguins based on bill length among male and female populations.


Bill Depth:    
Similarly, the significant association between sex and bill depth categories implies a meaningful divergence in the distribution of penguins with varying bill depths between male and female groups.
Flipper Length:

The chi-square test establishes a highly significant connection between sex and flipper length categories. This suggests a notable disparity in the distribution of penguins across small, medium, and large flipper length categories based on sex.

Conclusion:    
These results robustly support the hypothesis that there exists a compelling association between sex and body size categories among chinstrap penguins. The highly significant p-values underscore the practical importance of these findings, indicating substantial differences in the distribution of penguins across categorized size groups based on sex for the measured attributes.
## Interspecies comparison
...
knitr::kable(bird, col.names = c('Species','BM F', 'BM M', 'BM SDI', 'WL or FL F','WL or FL M','WL or FL SDI','Bias'),
      digits = 3, valign='t',align = c("l","r","r","r","r","r","c","l"))
```
...
Notably, the mean Sexual Dimorphism Index (SDI) for songbirds appears considerably lower when compared to other avian categories. This underscores the fact that the manifestation of sexual dimorphism extends beyond physical attributes alone.
